...
- **调用语义由入口与方法表达**：`PluginContext.handle` 同键唯一（工具、命令，描述符随 handler 一起存），`PluginContext.contribute` 类型级 0..N（收集式），两者都写进同一份类型注册表——工具与命令只是类型不同，不存在第二份注册表。同步派发匹配到 0 个即 `NO_HANDLER`，多个则按 `order` 升序**依次全部内联调用**并返回首个结果。需要聚合多个结果的调用点让请求自带结果容器承接，不依赖派发返回值。处理器失败即停止后续调用并把异常原样回传给调用者。
...
- **组合规则属于调用方**：注册表只保证「有序调用 + 收集返回值」，聚合 / 链式 / AND 由内核在各调用点决定，不存在按类型硬编码的调度参数。

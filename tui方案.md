...
### 11.10 后续调整（插件面板与五边版式，见 `tui插件方案.md`）

本文档 §3.3 写的「`DockElement` 三段式，`left` 位刻意空着」「`ChatShell.messageAreaRows(...)` 把浮层高度算进账本」
已被后续的插件 UI 工作改写，改动如下（原文保留，只在此处标注差异）：

| 原文位置 | 现在的口径 |
| --- | --- |
| §3.3「`DockElement` 三段式，`left` 位刻意空着」 | 改为**五边停靠**（`top` / `bottom` / `left` / `right` / `center`），四边都用于插件面板；所有约束仍一律 `length(n)`，**不用 `percent` / `fill`** |
| §3.3 / §4.1 `ChatShell.messageAreaRows` / `messageAreaWidth` | 拆成独立的 `ChatLayout`（二维账本，纯函数）：先底/顶、后左右、最后消息区。`ChatShell` 只把账本结论翻译成约束，不再自行推尺寸 |
| §4.1「`ChatShell.java`：版式（DockElement 三段式）」 | 版式改为「顶部面板 / 左右侧栏 / 停靠面板 + 浮层 + 输入区 + 状态栏」，底部各段顺序固定为 `[停靠面板, 浮层?, 输入区, 状态栏]` |
| §4.1 `ShellCommand.java`（只认 `/exit`） | 同时认 `/ui`（外壳自有命令：查看与切换插件区域），仍不进内核命令注册表 |

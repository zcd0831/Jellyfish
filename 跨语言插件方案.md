...
- 回调注册入口：按扩展点分组暴露（`tools()` 注册 `tool.provide` 处理器，`commands()` 注册 `command.provide` 处理器）。
- `events()`：订阅事件通道事件。
- `publisher()`：发布事件通道事件。
...
| 脚本侧 | Java 侧落点 | 说明 |
|------|------|------|
| `register_tool` | `PluginContext.tools().register(toolName, handler)` | 脚本贡献工具，与 Java 插件工具同构 |
| `register_command` | `PluginContext.commands().register(name, handler)` | 注册具名命令处理器 |

...
> ⚠️ **状态更新**：`PluginContext` 的入口与权限模型已经变化，现状以 `AGENTS.md` 为准。
> `tools()` / `commands()` / `events()` / `publisher()` 改为 `handle` / `contribute` / `observe` / `emit`；
> 「扩展点 ID + 能力声明」两层已删除，注册边界由**回调类型可见性**承载，出站发布不再门禁。
> 脚本侧的 JSON-RPC 消息类型与事件桥接机制不受影响，只需把落点换成新的四个入口。
...
graph TB
    subgraph JVM["JVM 进程（Java Agent Core）"]
        ...
        Bus["ExtensionRegistry + EventChannel<br>同步扩展点 + 异步通知<br>共用一份 TypeRegistry"]
...
### 1. PluginContext：能力边界

`PluginContext`（`jellyfish-api`）是插件能接触到的全部能力边界，也是脚本插件能力的**唯一来源**：

- `handle(回调类型, 路由键, 处理器, 选项)`：注册同键唯一的处理器（脚本的 `register_tool` / `register_command` 落到这里）。
- `contribute(回调类型, 处理器, 选项)`：注册类型级 0..N 的贡献。
- `observe(事件类型, 过滤器, 监听器)`：订阅事件通道事件。
- `emit(事件)`：发布事件通道事件。

设计意图：脚本不能直接启动进程，也不能直接访问事件总线，一切必须由桥接插件经 `PluginContext` 转发。
注册边界由**类型可见性**承载——插件拿不到的回调类型就注册不了，不需要声明清单。
...
- **脚本 → Java**：接收脚本的 `emit_event`，经 `PluginContext.emit(...)` 发布到事件通道。
...
- **命令载荷与 Java 插件完全一致**：脚本处理器收到的是 `CommandRequest`（命令名 + `CommandArguments`：`tokens` 切分结果 + `raw` 原文 + `sessionId`）；原文入口由内核切分、结构化入口（Web/TUI）由调用方直接给，脚本不必自己拆命令行。
...
| 脚本侧 | Java 侧落点 | 说明 |
|------|------|------|
| `register_tool` | `PluginContext.handle(ToolCallRequest.class, toolName, handler, options)` | 脚本贡献工具，与 Java 插件工具同构 |
| `register_command` | `PluginContext.handle(CommandRequest.class, name, descriptor, handler, options)` | 注册具名命令处理器（descriptor 是 `CommandDescriptor`：别名 / 说明 / 用法） |
| `subscribe_event` | `PluginContext.observe(...)` | 只订阅事件通道 |
| `emit_event` | `PluginContext.emit(...)` | 只发布事件，不发起回调 |
...
2. `ScriptGateway` 通过桥接插件在 `PluginContext.observe(...)` 上注册监听器。
...
1. 脚本调用 `emit_event`。
2. 网关收到后经 `PluginContext.emit(...)` 发布通知。
3. 标记来源为脚本，防止循环回推。
...
- 事件负载限制 1MB，超限只传事件标识。
- 推送队列有界，超出丢弃并告警。
- 插件不再需要声明事件权限（声明层已移除）；白名单与限流由 `EventBridge` 自身保证。
...
| Phase 3 | `EventBridge` 双向事件桥接，对齐 `EventChannel` 与 `ExtensionRegistry` |

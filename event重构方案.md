...
| P5 插件运行时接线 + 文档收尾 + 全量回归 | **已完成** | 新增 `PluginModule`（`PluginRuntimeConfig` / `PluginContextFactory` / `PF4JPluginManager`）并注册进 `JellyfishComponent`；`AgentHarness.bootstrap()` 末尾启动插件运行时，新增 `shutdown()`（先停插件再收敛通道）；`AGENTS.md` 与跨语言方案措辞同步修正；`mvn -o test` 全绿（api 42 + infra 348 + cli 8） |
